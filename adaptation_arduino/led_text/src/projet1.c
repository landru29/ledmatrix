...
const static unsigned char text[] = { 
  0b00000000, // space  
  0b00111110, // C
  0b01000001,
  0b01000001,
...
void loop() {
    ...
    for(i = 0; i < msg_length; i++) {
        printf("pos_by_matrix: %d - matrix: %d\n", pos_by_matrix, matrix);
        if (pos_by_matrix >= 31) {
            matrix++;
            chipSelect(matrix);
            ht1632c_send_bits(HT1632C_WRITE, 1 << 2);
        ...
        column_position = 0;
        if (display_buffer[letter_position] == ' ') {
            ht1632c_send_bits(0x00, 1<<7);
            letter_position++;
            pos_by_matrix++;
            continue;
        }
        get_letter_variables();
        while(column_position != letter_length) {
            char mychar = pgm_read_byte(aipointe_font + letter_offset + column_position);
    ...
    chipSelect(-1);
    /*
    ht1632c_send_command(HT1632_CMD_BLINK_ON, 0);
    usleep(10000000);
    ht1632c_send_command(HT1632_CMD_BLINK_OFF, 0);
    usleep(10000000);
    for(i=0; i<4; i++)
        ht1632c_send_command(HT1632_CMD_LEDOFF, i); 
    */
}
...
void get_letter_variables() {
  ...
  printf("letter_pos: %d\n", letter_position);
  letter_info = display_buffer[letter_position] - 33;
