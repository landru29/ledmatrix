...
{
  if (id<0) 
    digitalWrite(CS_ENABLE, LOW);
  else
    digitalWrite(CS_ENABLE, HIGH);
...
void setAddess(unsigned int addr, MATRIX* matrix)
{
  matrix->addr = addr & 0x7F;
  matrix->cs = (addr & 0x0380) >> 7; 
}


/*void writeData(unsigned int addr, unsigned char data, MATRIX* matrix)
{
  setAddress(addr, matrix);
  
  chipSelect(matrix->cs);

  ht1632c_send_bits(HT1632C_WRITE, 0b100);

  ht1632c_send_bits(matrix->addr, 0b1000000);
